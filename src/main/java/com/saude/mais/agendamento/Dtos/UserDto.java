...
public record UserDto(
    ...
    public  UserEntity toUserEntity() {
        UserDto user = this.cleanData();
        return new UserEntity(user.id(), user.firstName(), user.lastName(), user.gender(), user.username(), user.password(), user.email(), user.phone(), user.cpf(), user.userRole(), user.birthdate(), user.toAddressEntityList(), user.toHospitalEntityList());
    }


    public List<HospitalEntity> toHospitalEntityList(){
        List<HospitalEntity> hospitalsEntities = new ArrayList<>();

        hospitals.forEach((hospital) -> {
            hospitalsEntities.add(hospital.cleanData().toHospitalEntity());
        });
        return hospitalsEntities;
    }

    public List<AddressEntity> toAddressEntityList(){
        List<AddressEntity> addressEntities = new ArrayList<>();

        addresses.forEach(address -> {
            addressEntities.add(address.cleanData().toAddressEntity());
        });
